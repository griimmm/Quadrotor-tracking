...
class Drone:
    ...
    def arm(self):
        ...
        while not rospy.is_shutdown():
            now = rospy.get_time()
            if self.current_state.mode != "STABILIZED" and (now - prev_request > 2.):
                self.set_mode_client(base_mode=0, custom_mode="STABILIZED")
                prev_request = now 
            else:
    ...
    def takeoff(self, height):
        ...
        while self.pose[2] < height:
            ...
            self.rate.sleep()
    def yawrot(self, yaw1):
        print("rotating...")
        yaw1=(yaw1*np.pi)/180
        self.sp = self.pose
        q1=quaternion_from_euler(0,0,yaw1)
        tol=0.02
        (roll, pitch, yaw) = euler_from_quaternion (self.q)
        while norm(q1 - self.q) > tol:
            n = (q1 - self.q) / norm(q1 - self.q)
    ...
    def stop(self):
        while self.current_state.armed or self.current_state.mode == "STABILIZED":
            if self.current_state.armed:
                self.arming_client(False)
            if self.current_state.mode == "STABILIZED":
